...
The `route` label is the matched route TEMPLATE (e.g. "/users/{id}"), never the raw
path, so id values don't explode metric cardinality. These series feed your SLO and
...
def route_template(request) -> str:
    ...
    # Starlette routes expose `.path_format` (e.g. "/users/{id}").
