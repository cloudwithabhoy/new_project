...
//
// Istio's sidecars generate the spans; the application's only job is to FORWARD
// these headers on downstream calls. order fans out to cart/payment/inventory,
// so if we drop them the trace breaks across the whole checkout flow.
