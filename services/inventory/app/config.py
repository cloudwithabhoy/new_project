...
via a Secret — nothing is hardcoded. The Kafka wiring (brokers/topic/group) is read
the same way so the async consumer is configured without any code changes.
"""
...
def load_settings() -> Settings:
    return Settings(
        port=int(_getenv("PORT", "8088")),
        log_level=_getenv("LOG_LEVEL", "info").lower(),
        db_host=_getenv("DB_HOST", "localhost"),
        db_port=int(_getenv("DB_PORT", "5432")),
        db_user=_getenv("DB_USER", "inventory"),
        db_password=_getenv("DB_PASSWORD", "inventory"),
        db_name=_getenv("DB_NAME", "inventory"),
        ...
        kafka_group=_getenv("KAFKA_GROUP", "inventory"),
    )
