...
forward the propagation headers. notification is async-only (it consumes Kafka and
exposes a read-only inspection API), so it just surfaces the trace id into its logs
so a log line in Loki links back to the trace in Tempo.
"""
from __future__ import annotations

from contextvars import ContextVar

# Per-request / per-event trace id, set by the middleware (or consumer) and read by
# the log formatter.
